...
<input type="submit" name="submit" value="all1">
<input type="button" class="submit" value="all2">
<input type="button" class="submit" value="all2">
...
<?php
  ...
  {
     $sql = "SELECT isbn,`name`,publication,genere,permitions,issued FROM books";
     ...
     if($row > 0){
       $i=1;
        while($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<tr><td>".$row["name"]."</td><td>".$row["publication"]."</td><td>".$row["genere"]."</td><td>".$row["permitions"]."</td><td>".$row["issued"]."</td><td><input type='submit' class = 'issues' id = ".$row["isbn"]." value = 'issue'></td></tr>";
            $i++;
          }
    }
